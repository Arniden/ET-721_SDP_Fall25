...
class TestMainFunction(unittest.TestCase):
    ...
    # NEW TEST: user enters a non-numeric string for number of students ('Peter')
    @patch('builtins.input', side_effect=['Peter', '2', '80', '90'])
    @patch('sys.stdout', new_callable=io.StringIO)
    def test_non_numeric_number_of_students(self, mock_stdout, mock_input):
        studentsgrade.main()
        output = mock_stdout.getvalue()

        # Expect an error message for invalid number of students, then successful average
        self.assertIn("Invalid input. Please enter a positive number.", output)
        self.assertIn("The class average is 85.00", output)  # (80+90)/2 = 85.0

    # NEW TEST: user enters a non-numeric string for a grade ('Peter')
    @patch('builtins.input', side_effect=['2', 'Peter', '70', '85'])
    @patch('sys.stdout', new_callable=io.StringIO)
    def test_non_numeric_grade_input(self, mock_stdout, mock_input):
        studentsgrade.main()
        output = mock_stdout.getvalue()

        # Expect an error for the invalid grade input, then correct average
        self.assertIn("Invalid input! Enter a numerical value.", output)
        self.assertIn("The class average is 77.50", output)  # (70+85)/2 = 77.5
